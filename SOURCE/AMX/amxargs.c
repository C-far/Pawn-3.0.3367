 ...
 *
 *  Version: $Id: amxargs.c 3363 2005-07-23 09:03:29Z thiadmer $
 */
 ...
 */
static cell AMX_NATIVE_CALL n_argindex(AMX *amx, cell *params)
{
  const TCHAR *cmdline = rawcmdline();
  const TCHAR *option;
  int length, max;
 ...
 */
static cell AMX_NATIVE_CALL n_argstr(AMX *amx, cell *params)
{
 ...
 * returns true if the option was found and false otherwise
 */
static cell AMX_NATIVE_CALL n_argvalue(AMX *amx, cell *params)
{
  const TCHAR *option, *key;
  int length;
  cell *cptr;
...
/* argcount() */
static cell AMX_NATIVE_CALL n_argcount(AMX *amx, cell *params)
{
  const TCHAR *cmdline = rawcmdline();
  cell count = 0;
